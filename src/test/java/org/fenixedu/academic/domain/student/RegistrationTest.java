...
public class RegistrationTest {
    ...
    public void testGetEnrolmentsExecutionYearStream_matchesGetEnrolmentsExecutionYears() {
        Registration newRegistration = createRegistrationWithStudent("Student C", "studentC");
        StudentCurricularPlan newStudentCurricularPlan = newRegistration.getLastStudentCurricularPlan();

        EnrolmentTest.createEnrolment(newStudentCurricularPlan, executionInterval, findContext(), "studentC");
        EnrolmentTest.createEnrolment(newStudentCurricularPlan, nextYearFirstSemester, findContext(), "studentC");
    ...
    private static Registration createRegistrationWithStudent(final String name, final String username) {
        final Student student = StudentTest.createStudent(name, username);
    ...
    private static Context findContext() {
        return curricularCourse.getParentContextsSet().stream().filter(ctx -> ctx.isValid(executionInterval)).findAny()
                .orElseThrow();
    }
