...
public class RootCurriculumGroupTest {
    ...
    public static void init() {
        FenixFramework.getTransactionManager().withTransaction(() -> {
            ...
            addInternalCycle(root, twoCycleBachelorDcp, CycleType.SECOND_CYCLE);

            // Configuring conclusion rules on both cycles (isSkipValidation + credits limit) so isConcluded returns true
            final ProgramConclusion skipFirst = new ProgramConclusion("SKIP1", new LocalizedString(), new LocalizedString(),
                    new LocalizedString(), new LocalizedString(), true, false, true, null);
            final ProgramConclusion skipSecond = new ProgramConclusion("SKIP2", new LocalizedString(), new LocalizedString(),
                    new LocalizedString(), new LocalizedString(), true, false, true, null);
            ...
            new CreditsLimit(twoCycleBachelorDcp.getRoot().getCycleCourseGroup(CycleType.FIRST_CYCLE), null, executionYear, null, 0d,
                    0d);
    ...
    public void testRootCurriculumGroup_GetCycleCurriculumGroup() {
        assertNotNull(root.getCycleCurriculumGroup(CycleType.FIRST_CYCLE));
        assertNotNull(root.getCycleCurriculumGroup(CycleType.SECOND_CYCLE));
    ...
    public void testRootCurriculumGroup_GetFirstOrderedCycleCurriculumGroup() {
        ...
        assertSame(externalRoot.getCycleCurriculumGroup(CycleType.FIRST_CYCLE), externalRoot.getFirstOrderedCycleCurriculumGroup());
    }
    ...
    public void testRootCurriculumGroup_GetLastOrderedCycleCurriculumGroup() {
        ...
        // external root with one internal cycle: returns that cycle
        assertSame(externalRoot.getCycleCurriculumGroup(CycleType.FIRST_CYCLE), externalRoot.getLastOrderedCycleCurriculumGroup());
    }
    ...
    public void testRootCurriculumGroup_CreateCycle() {
        assertNotNull(root.getCycleCurriculumGroup(CycleType.FIRST_CYCLE));
        assertNotNull(root.getCycleCurriculumGroup(CycleType.SECOND_CYCLE));
        assertEquals(2, root.getCycleCurriculumGroups().size());

        assertNotNull(externalRoot.getCycleCurriculumGroup(CycleType.FIRST_CYCLE));
        assertNotNull(externalCycle);
        assertSame(externalCycle, externalRoot.getCycleCurriculumGroup(externalCycle.getCycleType()));

        final List<CycleCurriculumGroup> internalCycles = root.getInternalCycleCurriculumGroups();
    ...
    private static void addInternalCycle(final RootCurriculumGroup root, final DegreeCurricularPlan dcp,
            final CycleType cycleType) {
        CurriculumGroupFactory.createGroup(root, dcp.getRoot().getCycleCourseGroup(cycleType),
                executionYear.getFirstExecutionPeriod());
    }
