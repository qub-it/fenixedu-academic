...
public class RootCurriculumGroupTest {
    ...
    public static void init() {
        FenixFramework.getTransactionManager().withTransaction(() -> {
            ...
            // Conclusion rules on both cycles (isSkipValidation + credits limit)
            addCycleConclusionRules(twoCycleBachelorDcp.getRoot().getCycleCourseGroup(CycleType.FIRST_CYCLE));
            addCycleConclusionRules(twoCycleBachelorDcp.getRoot().getCycleCourseGroup(CycleType.SECOND_CYCLE));
    ...
    private static void addCycleConclusionRules(CycleCourseGroup cycleGroup) {
        final ProgramConclusion pc =
                new ProgramConclusion(cycleGroup.getCycleType().name() + "_SKIP", new LocalizedString(), new LocalizedString(),
                        new LocalizedString(), new LocalizedString(), true, false, true, null);
        cycleGroup.setProgramConclusion(pc);
        new CreditsLimit(cycleGroup, null, executionYear, null, 0d, 0d);
    }
